        ...
        <form role="form" id="ajax_form" action="{{ route('users.update',$user->id) }}" method="POST" enctype="multipart/form-data">
          <div class="box-body">
            ...
            <div class="form-group">
              ...
              <input type="text" name="name" class="form-control" id="admin_name" placeholder="Enter Employee Name" value="{{ $user->name }}" required="">
            </div>

            <div class="form-group">
              <label for="employee_id">Employee Id</label>
              <input type="text" data-inputmask="'mask': '999.999'" data-mask name="employee_id" class="form-control" id="employee_id" placeholder="Enter Employee id" value="{{ $user->employee_id }}" required="">
            ...
            <div class="form-group">
              ...
              <select name="group" class="form-control" required="">
              	...
              	<option value="watcher" {{ ($user->group == 'watcher')? 'selected': '' }}>watcher</option>
              	<option value="packer" {{ ($user->group == 'packer')? 'selected': '' }}>packer</option>
              	<option value="manager" {{ ($user->group == 'manager')? 'selected': '' }}>manager</option>
              </select>
            </div>

            <div class="form-group">
              <label for="">Assign Pages</label> <br>
              @foreach ($permissions as $permission)
                <label>
                  <input
                  @foreach ($user->permissions as $user_permission)
                    {{ ($user_permission->id == $permission->id) ? 'checked':'' }}
                  @endforeach
                   type="checkbox" name="permission[]" class="flat-red" value="{{ $permission->id }}">{{ $permission->page_name }}
                </label>
              @endforeach
            </div>
            ...
            <div class="form-group">
              ...
              <input type="file" id="image" name="image" class="form-control" onchange="document.getElementById('output').src = window.URL.createObjectURL(this.files[0])" accept="image/*">
              <div class="row">
                <div class="col-sm-6">
                    <img id="output" height="150px" width="auto" /><br>
                </div>
                <div class="col-sm-6">
                  <img src=" {{ asset('/storage/employees/'.$user->image) }}" height="200px" width="auto">
                </div>
              </div>
            </div>
