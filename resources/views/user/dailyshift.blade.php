...
<div class="container pt-3">
	<form method="POST" action="{{ route('postdailyShift') }}" id="DailyShiftForm">
		<div class="row">
			@csrf
			<div class="col-12">
			...
			<div class="col-4">
				...
				<select class="custom-select" name="shift" required="" id="Shift">
					@if (count($shifts) > 0)
				  		<option value="">Select Shift</option>
						  @foreach ($shifts as $shift)
						  	<option value="{{ $shift->shift_number }}">{{ $shift->shift_number }} ({{ $shift->shift_start}}-{{ $shift->shift_end }}) ({{ Carbon\Carbon::now()->format('d-m-Y') }})</option>
						  @endforeach
					@else
			...
			<div class="col-4">
				...
				<select class="custom-select" name="machine" required="" id="machine">
					@if (count($machineIdArr) > 0)
						<option value="">Select machine</option>
						@foreach ($machineIdArr as $machine)
			...
			<div class="col-4">
				...
				<input type="text" name="production_date" class="form-control d-inline pull-right" id="datepicker" value="">
			</div>

			<div class="col-6">
				<label for="item_group" class="font-weight-bold">Item Group</label>
		...
		<div class="row mt-3">
			...
			<div class="col-12 mb-5">
				<button type="submit" class="btn btn-success" id="ButtonConfirm">Add</button>
				<button type="reset" class="btn btn-danger allreset">Reset</button>
			</div>
